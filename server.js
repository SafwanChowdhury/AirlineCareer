...
app.get("/routes", (req, res) => {
  try {
    ...
    if (airline_id) {
      // Need to join with route_airlines to filter by airline_id
      query = query.replace(
        "FROM route_details",
        `
        FROM routes r
        JOIN airports dep ON r.departure_iata = dep.iata
        JOIN airports arr ON r.arrival_iata = arr.iata
        JOIN route_airlines ra ON r.id = ra.route_id
        JOIN airlines a ON ra.airline_id = a.id
      `
      );
...
app.get("/airports/:iata/routes", (req, res) => {
  try {
    ...
    if (airline_id || airline_name) {
      if (airline_id) {
        // Need to modify our query to join with route_airlines
        query = query.replace(
          "FROM route_details",
          `
          FROM routes r
